...
void Physics::update(Game *game)
{
	...
	while (spritesIt != sortedSweptShapes[LEFT_EDGE]->end())
	{
		CollidableObject *firstSprite = (*spritesIt);
		AABB *firstSweptBox = firstSprite->getSweptShape();
		float firstRightEdge = firstSweptBox->getRight();

		vector<CollidableObject*>::iterator secondSpriteIt = spritesIt + 1;

		while(secondSpriteIt != sortedSweptShapes[LEFT_EDGE]->end())
		{
			CollidableObject *secondSprite = (*secondSpriteIt);
			AABB *secondSweptBox = secondSprite->getSweptShape();
			float secondLeftEdge = secondSweptBox->getLeft();

			if(secondLeftEdge < firstRightEdge) //might be a collision, check vertical axis
			{
				float firstTopEdge = firstSweptBox->getTop();
				float secondBottomEdge = secondSweptBox->getBottom();

				float firstBottomEdge = firstSweptBox->getBottom();
				float secondTopEdge = secondSweptBox->getTop();

				if(secondBottomEdge < firstTopEdge || firstBottomEdge < secondTopEdge) //collision on two axes
				{
					unsigned int co1Edge, co2Edge;
					float timeUntilCollision = calculateTimeUntilCollision(firstSprite, secondSprite, co1Edge, co2Edge, 0.0f);
					if (timeUntilCollision > 1.0f)
					{
						//don't collide after all - at least not this frame
					}
					else
					{
						Collision *collisionToAdd = recycledCollisions.back();
						collisionToAdd->setCO1(firstSprite);
						collisionToAdd->setCO2(secondSprite);
						collisionToAdd->setCO1Edge(co1Edge);
						collisionToAdd->setCO2Edge(co2Edge);
					...
					//check next sprite, if applicable
					secondSpriteIt++;
				}
			}
			else
			{
				secondSpriteIt = sortedSweptShapes[LEFT_EDGE]->end();
			}
		}
		...
		for (int i = 0; i < world->getNumLayers(); i++)
		{
			...
			if (layer->hasCollidableTiles())
			{
				...
				if (test)
				{
					...
					pp->setHasDoubleJumpRemaining(true);
